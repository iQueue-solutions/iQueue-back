...
public class QueueService : IQueueService
{
    ...
    public async Task TransferAdminStatus(Guid queueId, Guid newAdminId, Guid userId)
    {
        var queue = await _unitOfWork.QueueRepository.GetByIdWithDetailsAsync(queueId);
        ...
        if (await _unitOfWork.UserRepository.GetByIdAsync(newAdminId) == null)
        {
            throw new QueueException("New admin not found.");
        }

        queue.AdminId = newAdminId;
        await _unitOfWork.SaveAsync();
    }
